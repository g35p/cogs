...
class Hangman(commands.Cog):
    ...
    async def hangman(self, ctx):
        ...
        while game:
            ...
            if t not in word:
                fails += 1
            if fails == len(self.man) - 2:
                game = False
                await ctx.send(f'😞 Ai pierdut! Cuvântul era: **{word}**.')
            if all(l in guessed for l in word if l in 'abcdefghijklmnopqrstuvwxyz'):
                game = False
                await ctx.send(f'🎉 Felicitări, ai câștigat! Cuvântul era: **{word}**.')
                await ctx.send(self.man[-1])
