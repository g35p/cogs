import discord
from redbot.core.data_manager
import bundled_data_path
from redbot.core.data_manager
import cog_data_path
from redbot.core
import commands
from redbot.core
import Config
...
boardmsg = None
check = lambda m: (
    m.channel == ctx.message.channel and m.author == ctx.message.author and len(m.content) == 1 and m.content.lower() in 'abcdefghijklmnopqrstuvwxyz'
)
while game:
...
if not(set('abcdefghijklmnopqrstuvwxyz') & set(word)) - set(guessed):
    p = self._get_message(word, guessed)
# Use the winning graphic by replacing `self.man[fails]`
with the new index
...
""
wordlists = [p.resolve().stem
    for p in cog_data_path(self).glob("*.txt")
]
if wordlists == []:
...
"
Set
if hangman messages should be one edited message or many individual messages.
